function printOutput(Results)
    ...
    for iElem = 1:size(stresses,3)
        ...
        fprintf('----------------------------------------------------------------------------------------------------------------------------------------------\n')
        fprintf('Intergration\t\tLocation\t\t\tLocation\t\t\t\t\t\t\tStrains\t\t\t\t\t\t\t\t\tStresses\n')
        fprintf('\tPoint\t\t\t(Natural)\t\t\t(Global)\t\t\t\t\te11\t\t\te22\t\t\te12\t\t\t\ts11\t\t\ts22\t\t\ts12\n')
        fprintf('----------------------------------------------------------------------------------------------------------------------------------------------\n')
        for iPoints = 1:size(naturalCoord,2)
            fprintf('%i\t\t\t(%8.4f%8.4f)\t\t(%8.4f%8.4f)\t\t', iPoints, [naturalCoord(1,iPoints,iElem),naturalCoord(2,iPoints,iElem),globalCoord(1,iPoints,iElem),globalCoord(2,iPoints,iElem)])
